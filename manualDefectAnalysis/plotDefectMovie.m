...
dirDataOutput = [mainDir,'\DefectData']; mkdir(dirDataOutput);
...
for k=frames,  % loop on all frames
    ...
    if or(or( size(gTruth.LabelData.h{k})>0, size(gTruth.LabelData.mh{k})>0),size(gTruth.LabelData.o{k})>0) % if we have any defects in thisFrame defects
        if size(gTruth.LabelData.h{k})>0 % read in all the half defects
        ...
        if size(gTruth.LabelData.mh{k})>0
            for j=1: length(gTruth.LabelData.mh{k}),% read in all the minus half defects
                numDefects(k) = numDefects(k)+1;
        ...
        if size(gTruth.LabelData.o{k})>0
            for j=1: length(gTruth.LabelData.o{k}),% read in all the one defects
                numDefects(k) = numDefects(k)+1;
   ...
   % now reorganize the defect strutcture for this frame according to
    ...
    cd(dirDataOutput); save(thisFileImNameBase, 'defect'); % save the organizaed defect structure for this frame
