...
% MODIFICATIONS BY US
% We modified this function to also calculate the coherence on the raw
% orientation field. We have an additional input "coherenceWinSize" for the
% size of the window for the coherence calculation and an additional output
% "coherence" 
% coherence = compute_coherence(orientim_orig, coherenceWinSize);
% function [orientim, reliability, coherence] = ...
%             ridgeorient_withCoherence(im, gradientsigma, blocksigma, orientsmoothsigma,coherenceWinSize)
...
function [orientim, reliability, coherence] = ...
    ...
    orientim_orig = pi/2 + atan2(sin2theta,cos2theta)/2;

%   KK CHANGE added coherence which is determined based on the original orientation
%   image without the additional smoothing (and hence has to be computed
%   within this function)
    coherence = compute_coherence(orientim_orig, round(coherenceWinSize));
