function runMovieAnalysis_temp (mainDir, toSave, calibration, numImages,isLS, frames)
...
coherenceWinSize=32*1.28/calibration;  %  Window size for the coherence calculation based on the  raw orientation field 
...
cohTH=0.82; % this is the threshold for the coherence in a region. This is determined from the local alignment of the raw orientation field (before the additional smoothing orientation field) and is used to define if there are ordered fibers in a given region
...
for k=frames,
    ...
    if isLS == 1
    createTotalMask_lightsheet(thisFile, mainDir, analysisParameters, toSave);
    else 
    createTotalMask(thisFile, mainDir, analysisParameters, toSave);
    end
...
toPlot= 1; % put 0 to not play the coherence movie; =1 if to plot coherence movies
% runMovieFracOrdered (mainDir, toSave, toPlot, frames); % this calculates the fraction ordered

% this version calculates the fraction ordered using thisCohDiskSize
thisCohTH=cohTH;
thisCohDiskSize=2*blocksigma; 
runMovieFracOrdered (mainDir, toSave, toPlot, frames, thisCohTH, thisCohDiskSize); % this calculates the fraction ordered