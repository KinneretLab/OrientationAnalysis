...
dirQuiver2 = [mainDir,'\QuiverFO']; % figures showing plots of quiver with fraction defined as ordered indicated on original image

cd(mainDir); mkdir(dirLocalOP); % make local OP folder
cd(mainDir); mkdir(dirQuiver); mkdir(dirQuiver2);% make quiver folders
...
try % load the image and raw analysis
    ...
    cd (dirReliability); load(thisFileImNameSave);
...
se1 = strel('disk',round(2*blocksigma)); se2 = strel('disk',round(localOPwinSize)); % this generates a disk of diameter 4*blocksigma for the first step and 2*localOPwinSize for the second step
...
Scoherence = simplifyMask (Scoherence, 2*blocksigma); % smooth the boundary of the mask with windowSize = 2*blocksigma
...
% if is4Images==0, ntot=1; end
% if is4Images==1, ntot=4; end % ntot is the number of images
...
for j=1:ntot,
    ...
    analysisRegion = zeros(size(nIm)); analysisRegion (~thisScoherence) = 20000; % in good region show original image; in regions outside good region show blue highlight
    maskedRegion = zeros(size(nIm)); maskedRegion (isnan(thisCoherence))= 20000 ; % in regions outside mask region show red highlight
    rgbImageWithAnalysisRegion {j}=  cat(3, maskedRegion, nIm, analysisRegion); % make rgb image r=in mask, g=original image, b=in analysis region
    %
    %% plot of quiver field (down sampled to 2*blocksigma) with local order parameter and original image with indication of region defined as ordered
    thisFOcoherence =  variableSubimage(FOcoherence, 2 ,ntot ,j); % take the jth subimage    
    % define analysis regions for RGB image - here using the simplified region with coherence>cohTH region as in FracOrdered analysis
    analysisRegion2 = zeros(size(nIm)); analysisRegion2 (~thisFOcoherence) = 20000; % in ordered region show original image; in regions outside good region show blue highlight
    rgbImageWithAnalysisRegion2 {j}=  cat(3, maskedRegion, nIm, analysisRegion2); % make rgb image r=in mask, g=original image, b=in analysis region
...
if ntot==1,
    ...
    % now plot quiver with original image after adaptive histogram with analysis region indicated
    quiverPlot2= cat(2, quiverImage2{1}, im2uint8(rgbImageWithAnalysisRegion2{1})); % image of the quiver side by side with original image with analysis regions
end
...
if ntot==2,
   ...
   % now plot quiver with original image after adaptive histogram with analysis region indicated
    rgbImageWithAnalysisAll2 = cat(2, im2uint8(rgbImageWithAnalysisRegion2{1}),im2uint8(rgbImageWithAnalysisRegion2{2})); % image of the quiver side by side with original image with analysis regions
     quiverPlot2 = cat(1, quiverImageAll, rgbImageWithAnalysisAll2); % image of the quiver side by side with original image with analysis regions
end
...
if ntot==4,
    ...
    rgbImageWithAnalysisAll = cat(2, im2uint8(rgbImageWithAnalysisRegion{1}),im2uint8(rgbImageWithAnalysisRegion{2}),im2uint8(rgbImageWithAnalysisRegion{3}),im2uint8(rgbImageWithAnalysisRegion{4})); % image of the quiver side by side with original image with analysis regions
     quiverPlot = cat(1, quiverImageAll, rgbImageWithAnalysisAll); % image of the quiver side by side with original image with analysis regions
   % now plot quiver with original image after adaptive histogram with analysis region indicated
    rgbImageWithAnalysisAll2 = cat(2, im2uint8(rgbImageWithAnalysisRegion2{1}),im2uint8(rgbImageWithAnalysisRegion2{2}),im2uint8(rgbImageWithAnalysisRegion2{3}),im2uint8(rgbImageWithAnalysisRegion2{4})); % image of the quiver side by side with original image with analysis regions
     quiverPlot2 = cat(1, quiverImageAll, rgbImageWithAnalysisAll2); % image of the quiver side by side with original image with analysis regions
end
...
if toSave==1,
    endName=strfind(thisFileImName,'.png');
    thisFileImNameSave = thisFileImName (1:endName-1);
    cd (dirLocalOP);save( [thisFileImNameSave,'.mat'], 'localOP','filtLocalOP','DSorientation','xVal','yVal','FOcoherence');
    cd(dirQuiver); imwrite(quiverPlot,[thisFileImNameSave,'.png']) % %
    cd(dirQuiver2); imwrite(quiverPlot2,[thisFileImNameSave,'.png']) % %
end
